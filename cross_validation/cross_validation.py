...
max_depth = [int(x) for x in np.linspace(2, 30, num=28)]
max_depth.append(None)

# Minimum number of samples required to split a node
min_samples_split = [int(x) for x in np.linspace(2, 30, num=28)]

# Minimum number of samples required at each leaf node
min_samples_leaf = [int(x) for x in np.linspace(2, 30, num=28)]
...
rf_random = RandomizedSearchCV(estimator=numeric_rf, param_distributions=random_grid, n_iter=500, cv=10, verbose=2, random_state=42, n_jobs=-1)
